...
1. 输入以下命令，在核函数实现文件matmul_leakyrelu的第114行增加断点，出现回显显示成功添加1个断点，如下所示。
   ...
   关键信息说明如下表：
   <table>
   <caption>表1 信息说明</caption>
   <thead><tr><th>字段</th><th>释义</th></tr></thead>
   <tbody><tr><td>device_debugdata</td><td>设备侧.o文件名。</td></tr><tr><td>matmul_leakyrelu_kernel.cpp</td><td>断点所在的Kernel函数名。</td></tr><tr><td>CopyOut</td><td>当前函数。</td></tr><tr><td>240</td><td>本次断点地址相对CopyOut函数的地址偏移量，即当前断点地址（0xff88）相对CopyOut函数所在地址的偏移量是240。</td></tr><tr><td>address = 0x000000000000ff88</td><td>断点的地址，即逻辑相对地址。</td></tr></tbody>
   </table>
   > **注意：**
