...
- 单击[Link](https://gitee.com/ascend/catlass)获取样例工程，为进行算子调试做准备。
...
1. 基于[前提条件](#zh-cn_topic_0000002370187089__section0949912192116)中的样例工程编译算子，获取可执行文件00_basic_matmul。  
  1. 修改example/CMakeLists.txt中的BISHENG_COMPILER_OPTIONS编译选项，将`-O2`修改为`-O0 -g`，并注释优化选项的`-DL2_CACHE_HINT`，使能编译器调试功能。  
    ```cpp
    ...
    set(BISHENG_COMPILER_OPTIONS
        -O0 -g -std=c++17 -xcce
        ...
        #-DL2_CACHE_HINT   # 使能编译器调试功能
    )
    ```
  2. 执行以下命令完成算子编译，编译完成后，在build/bin目录下生成可执行文件00_basic_matmul。  
    ```cpp
...
4. 运行算子程序，等待直到命中断点。  
  ...
  > 注意：`_ZN7Catlass13KernelAdapterINS_4Gemm6Kernel11BasicMatmulINS1_5Blo`为模板库的kernel名字，示例仅显示前面64位。
