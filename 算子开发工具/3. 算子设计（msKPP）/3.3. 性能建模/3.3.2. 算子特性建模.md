...
msKPP支持tensor拆分使用、debug模式和pipe信息的理论值与msprof实测值比对的功能，也支持对算子特性进行建模（搬运通路建模、随路转换建模和cache命中率建模），用户需根据实际需求进行选择，完成后可实现[算子计算搬运规格分析](https://www.hiascend.com/doc_center/source/zh/CANNCommunityEdition/82RC1/devaids/optool/atlasopdev_16_0010.html)、[极限性能分析](https://www.hiascend.com/doc_center/source/zh/CANNCommunityEdition/82RC1/devaids/optool/atlasopdev_16_0011.html)以及[算子tiling初步设计](https://www.hiascend.com/doc_center/source/zh/CANNCommunityEdition/82RC1/devaids/optool/atlasopdev_16_0012.html)。

> 文档中的Ascend*xxxyy*需替换为实际使用的处理器类型。

- 支持搬运通路建模（Atlas A2 训练系列产品/Atlas 800I A2 推理产品/A200I A2 Box 异构组件）  
  在Atlas A2 训练系列产品/Atlas 800I A2 推理产品/A200I A2 Box 异构组件中，新增了L1到fixpipe buffer(FP)的通路以及L1到Bias table(BT)的通路。前者用于L0C_TO_OUT随路转换时存储量化转换的scale参数，后者用于存储一维的bias数据。在本工具中对该搬运通路建模只需按照GM->L1->FP/BT的顺序即可。
  ```python
  ...
  ```
- 支持随路转换建模  
  在昇腾AI处理器的CUBE单元中，进行计算的数据格式需要是特殊的私有NZ格式。而通常在GM上的数据都是ND格式，因此在进行Cube运算时，需要将数据格式进行转换。在Atlas A2 训练系列产品/Atlas 800I A2 推理产品/A200I A2 Box 异构组件中，GM到CUBE相关的存储单元的搬运通路已具备ND转NZ的随路转换能力。  
...
- 支持PIPE信息的理论值与msprof实测值比对  
  ...
  **图1** Pipe_statistic.csv文件  
  ![图1 Pipe_statistic.csv文件](https://www.hiascend.com/doc_center/source/zh/CANNCommunityEdition/82RC1/devaids/optool/figure/zh-cn_image_0000002336349106.png "点击放大")