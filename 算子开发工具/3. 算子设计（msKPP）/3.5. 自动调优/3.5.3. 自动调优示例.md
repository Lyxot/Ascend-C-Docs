...
自动调优流程包括Kernel级自动调优和应用级自动调优两种，具体流程请参见[图1](#fig985071581517)，具体操作请参见[Kernel级自动调优](#section778122211315)和[应用级自动调优](#section14971258122)。

**图1** 自动调优流程示意图  
![自动调优流程示意图](https://www.hiascend.com/doc_center/source/zh/CANNCommunityEdition/82RC1/devaids/optool/figure/zh-cn_image_0000002336348954.png)

#### Kernel级自动调优

本章节以模板库catlass-v1-dev分支的[examples/00_basic_matmul](https://gitee.com/ascend/catlass/blob/catlass-v1-dev/examples/00_basic_matmul/basic_matmul.cpp)为例，介绍如何利用msKPP工具提供的[Python接口](https://www.hiascend.com/doc_center/source/zh/CANNCommunityEdition/82RC1/devaids/optool/atlasopdev_16_0156.html)实现Kernel级自动调优。
...
1. 完成算子Kernel开发后，Kernel函数的定义与实现将会呈现在basic_matmul.cpp文件中，如下所示。  
   ...
   // ...
   ```
2. 参考附录，在examples/00_basic_matmul目录中创建Python脚本文件[basic_matmul_autotune.py](https://www.hiascend.com/doc_center/source/zh/CANNCommunityEdition/82RC1/devaids/optool/atlasopdev_16_0157.html)与编译脚本文件[jit_build.sh](https://www.hiascend.com/doc_center/source/zh/CANNCommunityEdition/82RC1/devaids/optool/atlasopdev_16_0164.html)。  
...
5. 在算子代码程序basic_matmul.cpp中标识需调优的参数。  
   ...
   > ```
6. 通过[autotune](https://www.hiascend.com/doc_center/source/zh/CANNCommunityEdition/82RC1/devaids/optool/atlasopdev_16_0163.html)接口的configs入参定义参数搜索空间，每一类参数组合会替换算子Kernel代码中被标记的代码行，然后进行编译、运行并完成Kernel性能采集。搜索空间定义示例可参考如下所示。  
   > - 参数替换需合理，不能造成编译或运行错误。
   > - 参数替换原则如下（以configs中的第一行为例）：  
   >   1. 先替换`// tunable: L0Shape`方式标记的参数，将标记代码行（MatmulShape<128, 256, 64>）整行替换为configs中的value字符串（MatmulShape<128, 256, 64>）。  
   >   2. 再替换`// tunable`方式标记的代码行，将"="号后的MatmulShape<128, 256, 256>替换为configs中value字符串MatmulShape<64, 64, 64>。  
   ...
   ```
7. 执行[basic_matmul_autotune.py](https://www.hiascend.com/doc_center/source/zh/CANNCommunityEdition/82RC1/devaids/optool/atlasopdev_16_0157.html)文件运行算子，获得每种参数组合的耗时及最佳调优参数集合。以下仅展示可能的一种命令行输出结果。  
   ```
   # python3 basic_matmul_autotune.py 
...
本章节以模板库master分支的[examples/00_basic_matmul](https://gitee.com/ascend/catlass/blob/master/examples/00_basic_matmul/basic_matmul.cpp)为例，介绍如何利用msKPP工具提供的[Python接口](https://www.hiascend.com/doc_center/source/zh/CANNCommunityEdition/82RC1/devaids/optool/atlasopdev_16_0156.html)实现对应用级的自动调优。
...
1. 参考[examples/00_basic_matmul](https://gitee.com/ascend/catlass/blob/master/examples/00_basic_matmul/basic_matmul.cpp)示例，使用模板库Device层接口完成算子实现，并分别在115、117行末尾添加`// tunable`注释，用于替换"="号后的代码内容。  
   ...
   ...
   ```
2. 在[examples/00_basic_matmul](https://gitee.com/ascend/catlass/blob/master/examples/00_basic_matmul/basic_matmul.cpp)目录中创建Python脚本文件[basic_matmul_executable_autotune.py](https://www.hiascend.com/doc_center/source/zh/CANNCommunityEdition/82RC1/devaids/optool/atlasopdev_16_0183.html)与编译脚本文件[jit_build_executable.sh](https://www.hiascend.com/doc_center/source/zh/CANNCommunityEdition/82RC1/devaids/optool/atlasopdev_16_0182.html)。  

   可根据实际需要修改[basic_matmul_executable_autotune.py](https://www.hiascend.com/doc_center/source/zh/CANNCommunityEdition/82RC1/devaids/optool/atlasopdev_16_0183.html)脚本中[autotune_v2](https://www.hiascend.com/doc_center/source/zh/CANNCommunityEdition/82RC1/devaids/optool/atlasopdev_16_0171.html)接口传入的configs参数以搜索自定义tiling参数组合。
