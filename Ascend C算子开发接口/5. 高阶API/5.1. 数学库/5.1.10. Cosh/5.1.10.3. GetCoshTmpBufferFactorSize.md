...
推算示例如下：
- 算子实现需要调用Cosh接口，开发者为其预留currBuff大小的空间，利用GetCoshTmpBufferFactorSize接口得到maxLiveNodeCount、extraBuf输出值，可推导算子单次最大计算元素数量为：`currentShapeSize = (currBuff - extraBuf) / maxLiveNodeCount / typeSize`
...
<table>
  ...
  <tbody><tr><td>typeSize</td><td>输入</td><td>算子输入的数据类型大小，单位为字节。比如算子输入的数据类型为half，此处应传入2。</td></tr><tr><td>maxLiveNodeCount</td><td>输出</td><td>最大存活节点数，表示临时空间是单次计算数据量所占空间的多少倍。</td></tr><tr><td>extraBuffer</td><td>输出</td><td>使用的额外临时空间大小，单位为字节。</td></tr></tbody>
</table>
