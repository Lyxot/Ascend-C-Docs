...
通过该host侧接口设置算子输出的初始值，设置后会在算子执行前对算子输出的GM空间进行清零操作或者插入memset算子进行初始值设置。插入memset算子时，仅在入图场景下支持初始值设置任意值，单算子API执行的场景下仅支持清零。

**InitValue和**[SetNeedAtomic](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/82RC1/API/basicdataapi/atlasopapi_07_00240.html)**接口配合使用，SetNeedAtomic接口需要配置为true。**
...
<tbody><tr><td>value</td><td>输入</td><td><ul><li>uint64_t类型参数<p>仅支持输入0，输出参数调用该接口，会在算子执行前，对输出参数对应GM空间进行清零。</p></li><li>ScalarVar类型参数<p>ScalarVar用于指定输出参数初值的类型ScalarType和值ScalarNum，具体定义如下：</p><pre><code>enum class ScalarType : uint32_t {
  UINT64 = 0,
  INT64 = 1,
  UINT32 = 2,
  INT32 = 3,
...
};</code></pre><p>ScalarType当前仅支持UINT64/INT64/UINT32/INT32/UINT16/INT16/UINT8/INT8/FLOAT32/FLOAT16;</p><p>ScalarNum支持uint64_t/int64_t/float类型。</p><p>为方便使用，ScalarVar也支持立即数初始化，示例如下：</p><pre><code>InitValue({ScalarType::INT16, 1});</code></pre></li><li>const std::vector&lt;ScalarVar&gt; &amp;value类型<p>指定输出参数初值类型和值的列表，依次对应输出参数的数据类型和数据格式组合。</p></li></ul></td></tr></tbody>
</table>
...
- 针对OpParamDef &InitValue(const std::vector<ScalarVar> &value)接口输入value的size需要与输出参数配置的[DataType](https://www.hiascend.com/doc_center/source/zh/CANNCommunityEdition/82RC1/API/ascendcopapi/atlasascendc_api_07_0959.html)或[DataTypeList](https://www.hiascend.com/doc_center/source/zh/CANNCommunityEdition/82RC1/API/ascendcopapi/atlasascendc_api_07_0960.html)接口参数的size一致。同时，相同数据类型需保证设置的类型和值相同，否则将会报错。
