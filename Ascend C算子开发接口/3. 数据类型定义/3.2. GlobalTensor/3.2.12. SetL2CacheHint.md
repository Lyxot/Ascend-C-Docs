...
<table cellpadding="4" cellspacing="0" border="1">
  ...
  <tbody>
    <tr><td><strong>rwMode</strong></td><td>设置L2 Cache读写模式。
```cpp
enum CacheRwMode {
    READ = 1,
    WRITE = 2,
...
```
预留参数。为后续的功能做保留，开发者暂时无需关注，使用默认值即可。</td></tr>
  </tbody>
</table>
...
<table cellpadding="4" cellspacing="0" border="1">
  ...
  <tbody>
    <tr><td><strong>mode</strong></td><td>输入</td><td>用户指定的L2 Cache模式。
```cpp
...
```
如果用户在写算子时，相比不使能L2 Cache中，某GlobalTensor使能L2 Cache反而会导致实测性能下降，可以手动禁止该GlobalTensor使能L2 Cache。比如某算子仅会读一次某个GlobalTensor数据，数据进L2 Cache并不会对算子产生收益，反而会因为数据频繁的搬入L2 Cache造成性能损耗，可以考虑不使能该GlobalTensor L2 Cache能力。
如果不调用该接口，默认为CacheMode::CACHE_MODE_NORMAL，即GlobalTensor会使能L2 Cache。</td></tr>
  </tbody>
</table>
