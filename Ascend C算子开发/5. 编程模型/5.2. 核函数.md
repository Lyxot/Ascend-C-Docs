...
- **使用函数类型限定符**  
  ...
  ![图1 核函数、host侧执行函数、device侧执行函数调用关系](https://www.hiascend.com/doc_center/source/zh/CANNCommunityEdition/82RC1/opdevg/Ascendcopdevg/figure/zh-cn_image_0000002370255281.png)
- **使用变量类型限定符**  
...
模板核函数当前有如下使用限制：
- 仅支持<<<>>>调用方式。
- 暂不支持自定义数据类型。如下是一个反例，hello_world核函数尝试使用自定义数据类型Person，这种用法不被支持。
  ```cpp
  struct Person {
      int age;
  };
