...
在[算子实现](https://www.hiascend.com/doc_center/source/zh/CANNCommunityEdition/82RC1/opdevg/Ascendcopdevg/atlas_ascendc_10_0030.html)章节已经介绍了host侧tiling核心的实现方法，本章节侧重于介绍接入CANN框架时编程模式和API的使用。
...
图1 Tiling实现的输入输出  
![图1](https://www.hiascend.com/doc_center/source/zh/CANNCommunityEdition/82RC1/opdevg/Ascendcopdevg/figure/zh-cn_image_0000002336337286.png)
...
- **blockDim：**规定了核函数将会在几个核上执行。例如，需要计算8M的数据，每个核上计算1M的数据，blockDim设置为8，但是为了充分利用硬件资源，一般将blockDim设置为硬件平台的核数，根据核数进行数据切分。
  ...
  > 1. 耦合架构：由于其Vector、Cube单元是集成在一起的，blockDim用于设置启动多个AICore核实例执行，不区分Vector、Cube。AI Core的核数可以通过[GetCoreNumAiv](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/82RC1/API/ascendcopapi/atlasascendc_api_07_1031.html)或者[GetCoreNumAic](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/82RC1/API/ascendcopapi/atlasascendc_api_07_1030.html)获取。
  > 2. 分离架构  
  >   1. 针对仅包含Vector计算的算子，blockDim用于设置启动多少个Vector（AIV）实例执行，比如某款AI处理器上有40个Vector核，建议设置为40。  
  >   2. 针对仅包含Cube计算的算子，blockDim用于设置启动多少个Cube（AIC）实例执行，比如某款AI处理器上有20个Cube核，建议设置为20。  
  >   3. 针对Vector/Cube融合计算的算子，启动时，按照AIV和AIC组合启动，blockDim用于设置启动多少个组合执行，比如某款AI处理器上有40个Vector核和20个Cube核，一个组合是2个Vector核和1个Cube核，建议设置为20，此时会启动20个组合，即40个Vector核和20个Cube核。**注意：该场景下，设置的blockDim逻辑核的核数不能超过物理核（2个Vector核和1个Cube核组合为1个物理核）的核数。**  
  >   4. AIC/AIV的核数分别通过[GetCoreNumAic](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/82RC1/API/ascendcopapi/atlasascendc_api_07_1030.html)和[GetCoreNumAiv](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/82RC1/API/ascendcopapi/atlasascendc_api_07_1031.html)接口获取。
- **TilingKey（可选）**：TilingKey是一个算子内为了区分不同的实现而将kernel代码进行区分的方法，该方法类似于C++的Template模板机制，可减少不必要的icache miss以及scalar耗时，有助于优化单次调用kernel的性能。不同的kernel实现分支可以通过TilingKey来标识，host侧设置TilingKey后，可以选择对应的分支。例如，一个算子在不同的shape下，有不同的算法逻辑，kernel侧可以通过TilingKey来选择不同的算法逻辑，在host侧Tiling算法也有差异，host/kernel侧通过相同的TilingKey进行关联。  
  ...
  ```
  > 编译时，可以通过设置[--tiling_keys](https://www.hiascend.com/doc_center/source/zh/CANNCommunityEdition/82RC1/opdevg/Ascendcopdevg/atlas_ascendc_10_0068.html#ZH-CN_TOPIC_0000002336336606__li1753112716212)编译选项指定TilingKey，编译时只编译指定TilingKey相关的kernel代码，用于加速编译过程。
- **WorkspaceSize（可选）**：workspace是设备侧Global Memory上的一块内存。在Tiling函数中可以设置workspace的大小，框架侧会为其申请对应大小的设备侧Global Memory，在对应的算子kernel侧实现时可以使用这块workspace内存。  
  ...
  - Ascend C API需要预留workspace内存  
    API在计算过程需要一些workspace内存作为缓存，因此算子Tiling函数需要为API预留workspace内存，预留内存大小通过[GetLibApiWorkSpaceSize](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/82RC1/API/ascendcopapi/atlasascendc_api_07_1036.html)接口获取。
...
图2 Tiling开发流程图  
![图2](https://www.hiascend.com/doc_center/source/zh/CANNCommunityEdition/82RC1/opdevg/Ascendcopdevg/figure/zh-cn_image_0000002370175653.png)
...
3. TilingData结构定义，通过BEGIN_TILING_DATA_DEF接口定义一个TilingData的类，通过TILING_DATA_FIELD_DEF接口增加TilingData的两个字段totalLength、tileNum，通过END_TILING_DATA_DEF接口结束TilingData定义。相关接口的详细说明请参考[TilingData结构定义](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/82RC1/API/ascendcopapi/atlasascendc_api_07_1005.html)。
   ...
   END_TILING_DATA_DEF;
   ```
4. 注册TilingData结构，通过REGISTER_TILING_DATA_CLASS接口，注册TilingData类，和自定义算子相关联。REGISTER_TILING_DATA_CLASS第一个参数为op_type(算子类型)，本样例中传入AddCustom，第二个参数为TilingData的类名。REGISTER_TILING_DATA_CLASS接口介绍请参考[TilingData结构注册](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/82RC1/API/ascendcopapi/atlasascendc_api_07_1006.html)。
...
2. 设置TilingData。在步骤3中定义了TilingData类后，可以创建该类的一个实例，并通过调用set_{field_name}方法来设置各个字段值（其中field_name是步骤3中定义的tiling字段名）。设置完tiling字段后，通过调用SaveToBuffer方法完成TilingData实例的序列化和保存。
   ...
   1. 通过上下文获取输入输出shape信息。本样例中通过TilingContext的[GetInputShape](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/82RC1/API/basicdataapi/atlasopapi_07_00224.html)接口获取输入的shape大小。
   ...
   2. 设置TilingData。通过调用set_{field_name}方法来设置TilingData的字段值。
      ...
      tiling.set_tileNum(TILE_NUM);
      ```
   3. 调用TilingData类的SaveToBuffer接口完成序列化并保存至TilingContext上下文。SaveToBuffer的第一个参数为存储Buffer的首地址，第二个参数为Buffer的长度。通过调用[GetRawTilingData](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/82RC1/API/basicdataapi/atlasopapi_07_00243.html)获取无类型的TilingData的地址，再通过[GetData](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/82RC1/API/basicdataapi/atlasopapi_07_00257.html)获取数据指针，作为Buffer的首地址；通过调用[GetRawTilingData](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/82RC1/API/basicdataapi/atlasopapi_07_00243.html)获取无类型的TilingData的地址，再通过[GetCapacity](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/82RC1/API/basicdataapi/atlasopapi_07_00254.html)获取TilingData的长度，作为Buffer的长度。完成SaveToBuffer操作后需要通过[SetDataSize](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/82RC1/API/basicdataapi/atlasopapi_07_00256.html)设置TilingData的长度，该长度通过TilingData类的GetDataSize接口获取。
      ...
      context->GetRawTilingData()->SetDataSize(tiling.GetDataSize());
      ```
3. 通过[SetBlockDim](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/82RC1/API/basicdataapi/atlasopapi_07_00236.html)接口设置blockDim。
   
   ```cpp
   context->SetBlockDim(BLOCK_DIM);
   ```
4. （可选）通过[SetTilingKey](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/82RC1/API/basicdataapi/atlasopapi_07_00234.html)设置TilingKey。
   
   ```cpp
   context->SetTilingKey(1);
   ```
5. （可选）通过[GetWorkspaceSizes](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/82RC1/API/basicdataapi/atlasopapi_07_00244.html)获取workspace size指针，并设置size大小。此处仅作为举例，设置workspace的大小为0。
...
1. 使用C++语法定义TilingData结构体。  
   ...
   #endif // ADD_CUSTOM_TILING_H
   ```
2. Host侧tiling函数中对TilingData赋值。
   - 需要包含TilingData定义头文件。
   - 通过[GetTilingData](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/82RC1/API/basicdataapi/atlasopapi_07_00242.html)获取TilingData，并对其成员变量进行赋值。
   ...
   ```
3. Kernel侧通过[REGISTER_TILING_DEFAULT](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/82RC1/API/ascendcopapi/atlasascendc_api_07_00003.html)或者[REGISTER_TILING_FOR_TILINGKEY](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/82RC1/API/ascendcopapi/atlasascendc_api_07_00004.html)注册TilingData结构体，解析tiling数据至TilingData结构并使用，其中[REGISTER_TILING_DEFAULT](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/82RC1/API/ascendcopapi/atlasascendc_api_07_00003.html)同时也用于标识使用标准C++语法定义TilingData结构体。  
...
- GetTilingData获取的TilingData不包含初值，需显示赋值或在TilingData类中定义并调用赋值函数；
  ...
  static ge::graphStatus TilingFunc(gert::TilingContext* context)
  {
      TilingData * tiling = context->GetTilingData<TilingData>(); //获取TilingData结构体，此时totalLength、tileNum为0，并不会带入初始值
      ......
      // 需显示赋值
      tiling->totalLength = totalLength;  // 赋值TilingData成员变量
      tiling->tileNum = TILE_NUM;         // 赋值TilingData成员变量
...
在[TilingKey编程](#ZH-CN_TOPIC_0000002370175109__li578045965)章节介绍的TilingKey编程方式中，TilingKey不易于记忆和理解，因为它们通常是较长又没有明确含义的数字。
...
1. 在[自定义算子工程](https://www.hiascend.com/doc_center/source/zh/CANNCommunityEdition/82RC1/opdevg/Ascendcopdevg/atlas_ascendc_10_0058.html)的op_kernel目录下，新增定义模板参数和模板参数组合的头文件，本示例中头文件命名为tiling_key_add_custom.h。
   
   - 该头文件中需要包含模板头文件ascendc/host_api/tiling/template_argument.h。
   - 定义模板参数ASCENDC_TPL_ARGS_DECL和模板参数组合ASCENDC_TPL_ARGS_SEL（即可使用的模板）。具体API参考见[模板参数定义](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/82RC1/API/ascendcopapi/atlasascendc_api_07_00011.html)。
   ...
   ```
2. host侧调用[GET_TPL_TILING_KEY](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/82RC1/API/ascendcopapi/atlas_ascendc_10_00008.html)接口生成TilingKey。
