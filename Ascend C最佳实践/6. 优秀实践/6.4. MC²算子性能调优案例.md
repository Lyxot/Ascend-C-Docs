...
#### 案例介绍
MC^2^通算融合算子的性能收益主要来自于通信、计算的并行执行，即将输入数据切分为多个子块，子块的计算和通信任务形成两条流水线，通过两条流水线上任务的并行执行，实现流水掩盖，从而提升算子性能。如下图所示，MC^2^算子先做Matmul计算、后通信的场景，输入矩阵沿M轴被切分为两块，第二块数据的Matmul计算和第一块数据的通信可以并行执行，从而达到计算和通信时间相互掩盖的目的。本节的所有图示中MM代表Matmul计算，hcom代表通信任务。

![点击放大](https://www.hiascend.com/doc_center/source/zh/CANNCommunityEdition/82RC1/opdevg/ascendcbestP/figure/zh-cn_image_0000002370156745.png)

本案例将介绍如何分析通算融合算子的性能收益、如何制定较好的数据切分策略。更多MC^2^算子的完整样例请参考[MatmulAllReduce样例](https://gitee.com/ascend/samples/tree/v1.5-8.2.RC1/operator/ascendc/4_best_practices/23_matmul_all_reduce_custom)、[MatmulReduceScatter样例](https://gitee.com/ascend/samples/tree/v1.5-8.2.RC1/operator/ascendc/4_best_practices/22_matmul_reduce_scatter_custom)、[AllGatherMatmul样例](https://gitee.com/ascend/samples/tree/v1.5-8.2.RC1/operator/ascendc/4_best_practices/21_all_gather_matmul_custom)。

#### 获取性能数据
通过msProf算子调优工具获取算子性能数据：
- 获取[真实环境执行的性能数据](https://www.hiascend.com/doc_center/source/zh/CANNCommunityEdition/82RC1/opdevg/ascendcbestP/atlas_ascendc_best_practices_10_0008.html#ZH-CN_TOPIC_0000002370236353__section17953123893415)，包含各个流水的占比情况；
...
MC^2^算子收益 = (融合前算子串行耗时 - 融合后MC^2^算子耗时) / 融合前算子串行耗时

融合后MC^2^算子的执行耗时，受以下因素制约，从而影响算子性能收益。
- 因素一：计算和通信的执行时间差异  
  若计算和通信任务的执行时间相差不大，则融合后，MC^2^算子的计算和通信并行执行，能得到较好的流水掩盖，性能收益较大。  
  若计算和通信任务的执行时间差异较大，则融合后，MC^2^算子内计算和通信并行执行，能够掩盖的时间较少，算子整体执行耗时与未切分串行时的算子执行耗时接近，此时无法获得较大性能收益。  
  ![点击放大](https://www.hiascend.com/doc_center/source/zh/CANNCommunityEdition/82RC1/opdevg/ascendcbestP/figure/zh-cn_image_0000002370156757.png)
...
- 计算bound：  
  ...
  **图1** 计算bound示意图  
  ![图1 计算bound示意图](https://www.hiascend.com/doc_center/source/zh/CANNCommunityEdition/82RC1/opdevg/ascendcbestP/figure/zh-cn_image_0000002370156729.png)
- 通信bound：  
  对于同一个切分后的数据块，通信执行耗时大于计算执行耗时，此时，通信连续，且计算的头块要短，如下图所示。  
  **图2** 通信bound示意图  
  ![图2 通信bound示意图](https://www.hiascend.com/doc_center/source/zh/CANNCommunityEdition/82RC1/opdevg/ascendcbestP/figure/zh-cn_image_0000002370236569.png)
...
3. 将步骤2得到的数据块分别做AllReduce通信，利用msProf工具采集执行时间，得到每个数据块的执行时间t1, t2, ..., tn。然后，作图分析数据量与对应的执行时间的关系，拟合得到公式t = CostComm(m)，其中m表示数据块的M轴长度，t表示数据块的通信执行时间，CostComm表示拟合得到的m和t的映射关系。该映射关系一般为线性，若不满足线性关系，可以采用分段拟合的方式。示例如下：  
   ![通信拟合图1](https://www.hiascend.com/doc_center/source/zh/CANNCommunityEdition/82RC1/opdevg/ascendcbestP/figure/zh-cn_image_0000002336158588.png)  
   ![通信拟合图2](https://www.hiascend.com/doc_center/source/zh/CANNCommunityEdition/82RC1/opdevg/ascendcbestP/figure/zh-cn_image_0000002336158592.png)  
...
#### 总结
MC^2^算子通过数据切分后计算和通信的并行执行，获得性能收益，但受数据切分后执行时间膨胀的影响。对MC^2^算子进行性能调优的主要方式是制定数据切分策略，开发人员需要根据理论推导找到理想切分策略，然后根据实测结果调整，最终找到最优切分策略。